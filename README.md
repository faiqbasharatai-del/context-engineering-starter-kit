...
| **Watch** | `statusline/context-statusline.cjs` | A live context meter in your status bar: `CTX 82k/200k 41% [####------] FRESH`. You see rot coming instead of noticing it twenty sloppy messages later. (Claude Code CLI only — the VS Code extension sidebar doesn't render custom statuslines yet.) |
| **Guard** | `hooks/large-read-guard.cjs` | A PreToolUse hook and safety net: if anything tries to read a whole oversized file into your main context, it's stopped and pointed at the right move (slice it, grep it, or send a subagent). A capable model often greps a big file on its own — this is the backstop for when it (or a script, or you) doesn't. |
...
```bash
node .claude/tools/context-audit.cjs       # what does every session cost you?
```
